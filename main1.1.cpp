...
void setup()
{
  ...
  else
  {
    ...
    lv_scr_act();
    lv_obj_add_event_cb(objects.onmotor_bt, change_led_state, LV_EVENT_CLICKED, pLED1);
    lv_obj_add_event_cb(objects.offmotor_bt, change_led_state, LV_EVENT_CLICKED, pLED1);
    lv_obj_add_event_cb(objects.incrementpwm, pwm_spinbox_event_cb, LV_EVENT_PRESSING, NULL);
    lv_obj_add_event_cb(objects.decrementpwm, pwm_spinbox_event_cb, LV_EVENT_PRESSING, NULL);
    lv_obj_add_event_cb(objects.onpwm_sw, pwm_spinbox_event_cb, LV_EVENT_VALUE_CHANGED, NULL);

    lv_obj_add_event_cb(objects.onpwm_sw, get_value, LV_EVENT_VALUE_CHANGED, NULL);
    lv_timer_create(get_sw_state, 200, pSW1);

    lv_bar_set_value(objects.adc33bar, analogRead(33), LV_ANIM_ON);

    lv_led_set_brightness(objects.adc33led, analogRead(33) * 255 / 1023);

    lv_obj_add_event_cb(objects.nextapge_bt, control_page_event_cb, LV_EVENT_ALL, NULL);
    lv_obj_add_event_cb(objects.home_bt, control_page_event_cb, LV_EVENT_ALL, NULL);
